			...
			if t.phase == mapPhase {
				...
				for i := range fs {
				 ...
				 for i :=0; i<t.nMap; i++ {
					...
					}// Intermediate File open

					dec := json.NewDecoder(file) //decode file
					...
					for{
						...
						//log.Println("value of kv ", kv)
						_, ok := keyValues[kv.Key]
						if !ok {
							keyValues[kv.Key] = []string{}
						}
						keyValues[kv.Key] = append(keyValues[kv.Key], kv.Value)
					}
					file.Close()
				 }
				 newfile, newBuffer := CreateFileAndBuf(mergeName(t.dataDir, t.jobName, t.taskNumber))
				 for key, value := range keyValues{
				 	WriteToBuf(newBuffer, t.reduceF(key, value))
				 }
				 SafeClose(newfile, newBuffer)
			}
			t.wg.Done()
		case <-c.exit:
			return
...
func (c *MRCluster) run(jobName, dataDir string, mapF MapF, reduceF ReduceF, mapFiles []string, nReduce int, notify chan<- []string) {
	...
	//output result file
	fileName := make([]string, 0, nReduce)
	for i:= 0; i < nReduce; i++ {
		fileName = append(fileName, mergeName(dataDir, jobName, i))
	}
	notify <- fileName
}
